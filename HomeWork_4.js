/* Создать коллекцию products и заполнить документами (6 штук) со следующими 
свойствами (_id, title, price, count). Используйте следующие данные
...
db.products.updateOne(
    { _id: 2 },
    { $push: { country: { $each: ["Япония", "Корея", "Китай"] }}} 
)

8. Для товара "Ролики" убрать поставщика Америка
db.products.updateOne(
    { title: "Ролики" }, 
    { $pull: { country: "Америка" } } 
)
...
db.products.updateMany(
    {}, 
    { $push: { country: { $each: ["Франция", "Италия"] }}}
)
